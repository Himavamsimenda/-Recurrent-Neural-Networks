...
class RNN:
    ...
    def forward(self, X, a_prev):
        ...
        for t in range(len(X)):  
            ...
            if (self.X[t] != None):
                xt[self.X[t]] = 1
            x[t] = xt
            a[t] = np.tanh(np.dot(self.Waa, a[t - 1]) + np.dot(self.Wax, x[t]) + self.ba)
    ...
    def backward(self,x, a, y_preds, targets):
        ...
        for t in reversed(range(len(x))):
            ...
            da = np.dot(self.Wya.T, dy) + np.dot(self.Wya.T, dy)
            daraw = (1 - a[t] ** 2) * da
            self.dba += daraw
            self.dWax += np.dot(daraw, x[t].T)
            self.dWaa += np.dot(daraw, a[t - 1].T)
            da_next = daraw
            self.dWya += np.dot(dy, a[t].T)
    ...
    def loss(self, y_preds, targets):
        ...
        loss = -np.sum(np.log([y_preds[t][targets[t]] for t in range(len(y_preds))]))
        ### STUDENT CODE ENDS HERE
    ...
    def predict(self, start):
        ...
        txt = ""
        while counter < max_chars and idx != newline_character:
            a_next = np.tanh(np.dot(self.Wax, x) + np.dot(self.Waa, a_prev) + self.ba)
            y_pred = self.softmax(np.dot(self.Wya, a_next) + self.by)
